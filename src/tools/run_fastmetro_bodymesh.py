...
from src.utils.renderer_opendr import OpenDR_Renderer, visualize_reconstruction_opendr, \
    visualize_reconstruction_multi_view_opendr, visualize_reconstruction_smpl_opendr

try:
    from src.utils.renderer_pyrender import PyRender_Renderer, visualize_reconstruction_pyrender, \
        visualize_reconstruction_multi_view_pyrender, visualize_reconstruction_smpl_pyrender
except:
...
def smpl_param_loss(criterion_smpl, pred_rotmat, pred_betas, gt_pose, gt_betas, has_smpl, device):
    ...
    if len(gt_rotmat_with_shape) > 0:
        loss = criterion_smpl(pred_rotmat_with_shape, gt_rotmat_with_shape) + 0.1 * criterion_smpl(
            pred_betas_with_shape, gt_betas_with_shape)
    else:
...
class EdgeLengthGTLoss(torch.nn.Module):
    ...
    def forward(self, pred_vertices, gt_vertices, has_smpl, device):
        ...
        if len(coord_gt) > 0:
            if self.uniform:
                d1_out = torch.sqrt(torch.sum((coord_out[:, edge[:, 0], :] - coord_out[:, edge[:, 1], :]) ** 2, 2,
                                              keepdim=True) + 1e-8)  # batch_size X num_edges X 1
                d1_gt = torch.sqrt(torch.sum((coord_gt[:, edge[:, 0], :] - coord_gt[:, edge[:, 1], :]) ** 2, 2,
                                             keepdim=True) + 1e-8)  # batch_size X num_edges X 1
                edge_diff = torch.abs(d1_out - d1_gt)
            else:
                d1_out = torch.sqrt(torch.sum((coord_out[:, face[:, 0], :] - coord_out[:, face[:, 1], :]) ** 2, 2,
                                              keepdim=True) + 1e-8)  # batch_size X num_faces X 1
                d2_out = torch.sqrt(torch.sum((coord_out[:, face[:, 0], :] - coord_out[:, face[:, 2], :]) ** 2, 2,
                                              keepdim=True) + 1e-8)  # batch_size X num_faces X 1
                d3_out = torch.sqrt(torch.sum((coord_out[:, face[:, 1], :] - coord_out[:, face[:, 2], :]) ** 2, 2,
                                              keepdim=True) + 1e-8)  # batch_size X num_faces X 1

                d1_gt = torch.sqrt(torch.sum((coord_gt[:, face[:, 0], :] - coord_gt[:, face[:, 1], :]) ** 2, 2,
                                             keepdim=True) + 1e-8)  # batch_size X num_faces X 1
                d2_gt = torch.sqrt(torch.sum((coord_gt[:, face[:, 0], :] - coord_gt[:, face[:, 2], :]) ** 2, 2,
                                             keepdim=True) + 1e-8)  # batch_size X num_faces X 1
                d3_gt = torch.sqrt(torch.sum((coord_gt[:, face[:, 1], :] - coord_gt[:, face[:, 2], :]) ** 2, 2,
                                             keepdim=True) + 1e-8)  # batch_size X num_faces X 1
...
class EdgeLengthSelfLoss(torch.nn.Module):
    ...
    def forward(self, pred_vertices, has_smpl, device):
        ...
        if len(coord_out) > 0:
            if self.uniform:
                edge_self_diff = torch.sqrt(
                    torch.sum((coord_out[:, edge[:, 0], :] - coord_out[:, edge[:, 1], :]) ** 2, 2,
                              keepdim=True) + 1e-8)  # batch_size X num_edges X 1
            else:
                d1_out = torch.sqrt(torch.sum((coord_out[:, face[:, 0], :] - coord_out[:, face[:, 1], :]) ** 2, 2,
                                              keepdim=True) + 1e-8)  # batch_size X num_faces X 1
...
def run_train(args, train_dataloader, val_dataloader, FastMETRO_model, smpl, mesh_sampler, renderer,
              smpl_intermediate_faces, smpl_intermediate_edges):
    smpl.eval()
    ...
    if args.resume_epoch > 0:
        log_eval_metrics_mpjpe.set(epoch=args.resume_mpjpe_best_epoch, mPVPE=args.resume_mpjpe_best_mpvpe,
                                   mPJPE=args.resume_mpjpe_best_mpjpe, PAmPJPE=args.resume_mpjpe_best_pampjpe)
        log_eval_metrics_pampjpe.set(epoch=args.resume_pampjpe_best_epoch, mPVPE=args.resume_pampjpe_best_mpvpe,
                                     mPJPE=args.resume_pampjpe_best_mpjpe, PAmPJPE=args.resume_pampjpe_best_pampjpe)
    ...
    for _, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        gt_3d_vertices_intermediate = gt_3d_vertices_intermediate - gt_smpl_3d_pelvis[:, None,
                                                                    :]  # batch_size X 1723 X 3
        ...
        pred_3d_vertices_intermediate = pred_3d_vertices_intermediate - pred_3d_joints_from_smpl_pelvis[:, None,
                                                                        :]  # batch_size X 1723 X 3
        pred_3d_vertices_coarse = pred_3d_vertices_coarse - pred_3d_joints_from_smpl_pelvis[:, None,
                                                            :]  # batch_size X 431 X 3
        ...
        pred_3d_joints_from_token = pred_3d_joints_from_token - pred_3d_joints_from_token_pelvis[:, None,
                                                                :]  # batch_size X 14 X 3
        ...
        if args.use_smpl_param_regressor:
            ...
            pred_smpl_2d_joints = orthographic_projection(pred_smpl_3d_joints,
                                                          pred_cam.clone().detach())  # batch_size X 14 X 2
            loss_smpl_3d_joints = keypoint_3d_loss(criterion_3d_keypoints, pred_smpl_3d_joints, gt_3d_joints,
                                                   has_3d_joints, args.device)
            loss_smpl_2d_joints = keypoint_2d_loss(criterion_2d_keypoints, pred_smpl_2d_joints, gt_2d_joints,
                                                   has_2d_joints)
            loss_smpl_vertices = vertices_loss(criterion_3d_vertices, pred_smpl_3d_vertices, gt_3d_vertices_fine,
                                               has_smpl, args.device)
            # compute smpl parameter loss
            loss_smpl = smpl_param_loss(criterion_smpl_param, pred_rotmat, pred_betas, gt_pose, gt_betas, has_smpl,
                                        args.device) + loss_smpl_3d_joints + loss_smpl_2d_joints + loss_smpl_vertices
        ...
        if ((iteration == 10) or (iteration == 100) or ((iteration % args.logging_steps) == 0) or (
                iteration == max_iter)) and is_main_process():
            logger.info(
                ' '.join(
                ...
                + ' loss: {:.4f}, 3D-joint-loss: {:.4f}, 3D-vertex-loss: {:.4f}, edge-normal-loss: {:.4f}, 2D-joint-loss: {:.4f}, lr: {:.6f}'.format(
                    log_losses.avg, log_loss_3d_joints.avg, log_loss_3d_vertices.avg, log_loss_edge_normal.avg,
        ...
        if (iteration % iters_per_epoch) == 0:
            ...
            val_mPVPE, val_mPJPE, val_PAmPJPE = val_result['val_mPVPE'], val_result['val_mPJPE'], val_result[
                'val_PAmPJPE']
            if args.use_smpl_param_regressor:
                val_smpl_mPVPE, val_smpl_mPJPE, val_smpl_PAmPJPE = val_result['val_smpl_mPVPE'], val_result[
                    'val_smpl_mPJPE'], val_result['val_smpl_PAmPJPE']
            ...
            if is_main_process():
                if args.use_smpl_param_regressor:
                    logger.info(
                        ...
                        + '  MPVPE: {:6.2f}, MPJPE: {:6.2f}, PA-MPJPE: {:6.2f} '.format(1000 * val_mPVPE,
                                                                                        1000 * val_mPJPE,
                                                                                        1000 * val_PAmPJPE)
                        + '  <SMPL> MPVPE: {:6.2f}, <SMPL> MPJPE: {:6.2f}, <SMPL> PA-MPJPE: {:6.2f} '.format(
                            1000 * val_smpl_mPVPE, 1000 * val_smpl_mPJPE, 1000 * val_smpl_PAmPJPE)
                    )
                else:
                    logger.info(
                        ' '.join(['Validation', 'Epoch: {ep}', ]).format(ep=epoch)
                ...
                logger.info(
                    ...
                    + ' <MPVPE> {:6.2f} <MPJPE> {:6.2f}, <PA-MPJPE> {:6.2f}, at Epoch {:6.2f}'.format(
                        1000 * log_eval_metrics_pampjpe.mPVPE, 1000 * log_eval_metrics_pampjpe.mPJPE,
                        1000 * log_eval_metrics_pampjpe.PAmPJPE, log_eval_metrics_pampjpe.epoch)
                )
                logger.info(
                    'Best Results (MPJPE):'
                    + ' <MPVPE> {:6.2f}, <MPJPE> {:6.2f}, <PA-MPJPE> {:6.2f}, at Epoch {:6.2f}'.format(
                        1000 * log_eval_metrics_mpjpe.mPVPE, 1000 * log_eval_metrics_mpjpe.mPJPE,
                        1000 * log_eval_metrics_mpjpe.PAmPJPE, log_eval_metrics_mpjpe.epoch)
                )
    ...
    if args.use_smpl_param_regressor:
        ...
        logger.info(
            ...
            + '  MPVPE: {:6.2f}, MPJPE: {:6.2f}, PA-MPJPE: {:6.2f} '.format(1000 * val_mPVPE, 1000 * val_mPJPE,
                                                                            1000 * val_PAmPJPE)
            + '  <SMPL> MPVPE: {:6.2f}, <SMPL> MPJPE: {:6.2f}, <SMPL> PA-MPJPE: {:6.2f} '.format(1000 * val_smpl_mPVPE,
                                                                                                 1000 * val_smpl_mPJPE,
                                                                                                 1000 * val_smpl_PAmPJPE)
        )
    else:
        logger.info(
            ' '.join(['Validation', 'Epoch: {ep}', ]).format(ep=epoch)
...
def main(args):
    ...
    mesh_sampler = Mesh()
    smpl_intermediate_faces = torch.from_numpy(
        np.load('./src/modeling/data/smpl_1723_faces.npy', encoding='latin1', allow_pickle=True).astype(np.int64)).to(
        args.device)
    smpl_intermediate_edges = torch.from_numpy(
        np.load('./src/modeling/data/smpl_1723_edges.npy', encoding='latin1', allow_pickle=True).astype(np.int64)).to(
        args.device)
    ...
    val_dataloader = make_data_loader(args, args.val_yaml, args.distributed, is_train=False,
                                      scale_factor=args.img_scale_factor)
    if args.run_evaluation:
        run_eval(args, val_dataloader, _FastMETRO_Network, smpl, renderer)
    else:
        train_dataloader = make_data_loader(args, args.train_yaml, args.distributed, is_train=True,
                                            scale_factor=args.img_scale_factor)
        run_train(args, train_dataloader, val_dataloader, _FastMETRO_Network, smpl, mesh_sampler, renderer,
                  smpl_intermediate_faces, smpl_intermediate_edges)
